...
describe('taskLifecycleService', () => {
  it('returns no actions while planning', () => {
    expect(taskLifecycleService.getTaskLifecycleActions(createTask('planning'))).toEqual([]);
  });

  it('returns confirmation actions while awaiting confirmation', () => {
    expect(taskLifecycleService.getTaskLifecycleActions(createTask('awaiting_confirmation'))).toEqual([
  ...
  it('returns only complete action while awaiting review', () => {
    expect(taskLifecycleService.getTaskLifecycleActions(createTask('awaiting_review'))).toEqual([
