...
export default function TaskDecisionPrompt({
  actions,
  onAction,
}: TaskDecisionPromptProps) {
  if (actions.length === 0) {
    return null;
  }
  ...
  return (
    <section className="mx-3 mb-3 mt-0 rounded-[24px] border border-amber-400/26 bg-[linear-gradient(180deg,rgba(71,37,9,0.36),rgba(40,22,10,0.28))] px-4 py-4 text-slate-200 shadow-[0_18px_50px_rgba(0,0,0,0.24)] md:mx-5">
      <div className="flex flex-col gap-3">
        <div>
          <p className="m-0 text-xs font-semibold uppercase tracking-[0.14em] text-amber-300">
            確認待ち
          </p>
          <h4 className="m-0 mt-1 text-base font-semibold text-slate-100">
            計画を確認して次の操作を選んでください
          </h4>
